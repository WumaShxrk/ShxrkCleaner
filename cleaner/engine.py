...
def clean_epic_cache():
    local = os.environ.get("LOCALAPPDATA", "")
    targets = []

    epic_saved = os.path.join(local, "EpicGamesLauncher", "Saved")
    for sub in ("HttpCache", "Logs", "Crashes", "webcache", "webcache_4147", "webcache_4430"):
        p = os.path.join(epic_saved, sub)
        if os.path.isdir(p):
            targets.append(p)

    if not targets:
        warning("No Epic Games cache found")
        return {"freed": 0, "files": 0, "ok": True}

    detail(f"Found {len(targets)} Epic Games cache directories")
    freed, count = _parallel_delete_trees(targets)
    success(f"Epic Games cache cleaned — {format_size(freed)} from {count:,} items")
    return {"freed": freed, "files": count, "ok": True}


def clean_riot_cache():
    local = os.environ.get("LOCALAPPDATA", "")
    program_data = os.environ.get("ProgramData", r"C:\ProgramData")

    targets = []

    riot_local = os.path.join(local, "Riot Games")
    if os.path.isdir(riot_local):
        try:
            for root, dirs, files in os.walk(riot_local):
                if "Logs" in dirs:
                    targets.append(os.path.join(root, "Logs"))
                if "Crashes" in dirs:
                    targets.append(os.path.join(root, "Crashes"))
        except (PermissionError, OSError):
            pass

    riot_pd = os.path.join(program_data, "Riot Games")
    if os.path.isdir(riot_pd):
        try:
            for root, dirs, files in os.walk(riot_pd):
                if "Logs" in dirs:
                    targets.append(os.path.join(root, "Logs"))
        except (PermissionError, OSError):
            pass

    riot_client_logs = r"C:\Riot Games\Riot Client\Logs"
    if os.path.isdir(riot_client_logs):
        targets.append(riot_client_logs)

    if not targets:
        warning("No Riot Games logs/cache found")
        return {"freed": 0, "files": 0, "ok": True}

    detail(f"Found {len(targets)} Riot Games log/cache directories")
    freed, count = _parallel_delete_trees(targets)
    success(f"Riot Games cache cleaned — {format_size(freed)} from {count:,} items")
    return {"freed": freed, "files": count, "ok": True}
...
def clean_ea_cache():
    ...
    # EA App
    ea_local = os.path.join(local, "Electronic Arts", "EA Desktop")
    for sub in ("Logs", "Cache"):
        p = os.path.join(ea_local, sub)
        if os.path.isdir(p):
            targets.append(p)
            
    ea_pd = os.path.join(program_data, "EA Desktop")
    if os.path.isdir(ea_pd):
        for sub in ("Logs", "Cache"):
            p = os.path.join(ea_pd, sub)
            if os.path.isdir(p):
                targets.append(p)
                
    # Origin (Legacy)
    for p in [os.path.join(local, "Origin"), os.path.join(appdata, "Origin")]:
        if os.path.isdir(p):
            try:
                for entry in os.scandir(p):
                    if entry.is_dir() and entry.name.lower() != "localcontent":
                        targets.append(entry.path)
            except (PermissionError, OSError):
                pass
                
    origin_pd = os.path.join(program_data, "Origin")
    if os.path.isdir(origin_pd):
        for sub in ("Logs", "CustomAvatars", "WebCache"):
            p = os.path.join(origin_pd, sub)
            if os.path.isdir(p):
                targets.append(p)

    if not targets:
        warning("No EA/Origin cache found")
        return {"freed": 0, "files": 0, "ok": True}

    detail(f"Found {len(targets)} EA/Origin cache directories")
    freed, count = _parallel_delete_trees(targets)
    success(f"EA/Origin cache cleaned — {format_size(freed)} from {count:,} items")
    return {"freed": freed, "files": count, "ok": True}
...
def clean_battlenet_cache():
    ...
    targets = []
    candidates = [
        os.path.join(program_data, "Battle.net"),
        os.path.join(program_data, "Blizzard Entertainment"),
        os.path.join(local, "Battle.net"),
        os.path.join(local, "Blizzard Entertainment"),
        ...
        if os.path.isdir(c):
            try:
                for entry in os.scandir(c):
                    name_lower = entry.name.lower()
                    if entry.is_dir() and name_lower in ("cache", "logs", "browsercache", "webcache", "errors", "dump", "telemetry"):
                        targets.append(entry.path)
            except (PermissionError, OSError):
                pass
                
    if not targets:
        warning("No Battle.net cache found")
        return {"freed": 0, "files": 0, "ok": True}

    detail(f"Found {len(targets)} Battle.net cache directories")
    freed, count = _parallel_delete_trees(targets)
    success(f"Battle.net cache cleaned — {format_size(freed)} from {count:,} items")
    return {"freed": freed, "files": count, "ok": True}
...
def clean_ubisoft_cache():
    ...
    ubi_local = os.path.join(local, "Ubisoft Game Launcher")
    if os.path.isdir(ubi_local):
        for sub in ("cache", "logs", "crashes", "spool"):
            p = os.path.join(ubi_local, sub)
            if os.path.isdir(p):
                targets.append(p)
                
    ubi_pf = os.path.join(program_files, "Ubisoft", "Ubisoft Game Launcher")
    if os.path.isdir(ubi_pf):
        for sub in ("cache", "logs", "crashes"):
            p = os.path.join(ubi_pf, sub)
            if os.path.isdir(p):
                targets.append(p)
                
    if not targets:
        warning("No Ubisoft Connect cache found")
        return {"freed": 0, "files": 0, "ok": True}

    detail(f"Found {len(targets)} Ubisoft cache directories")
    freed, count = _parallel_delete_trees(targets)
    success(f"Ubisoft Connect cache cleaned — {format_size(freed)} from {count:,} items")
    return {"freed": freed, "files": count, "ok": True}


def clean_whatsapp_cache():
    local = os.environ.get("LOCALAPPDATA", "")
    
    targets = []
    
    wa_local = os.path.join(local, "WhatsApp")
    for sub in ("Cache", "Code Cache", "GPUCache", "logs"):
        p = os.path.join(wa_local, sub)
        if os.path.isdir(p):
            targets.append(p)
        ...
        try:
            for entry in os.scandir(uwp_base):
                if entry.is_dir() and "whatsapp" in entry.name.lower():
                    ...
                    if os.path.isdir(lc):
                        targets.append(lc)
                    ac = os.path.join(entry.path, "AC", "INetCache")
                    if os.path.isdir(ac):
                        targets.append(ac)
        except (PermissionError, OSError):
            pass
    ...
    if not targets:
        warning("No WhatsApp cache found")
        return {"freed": 0, "files": 0, "ok": True}

    detail(f"Found {len(targets)} WhatsApp cache directories")
    freed, count = _parallel_delete_trees(targets)
    success(f"WhatsApp cache cleaned — {format_size(freed)} from {count:,} items")
    return {"freed": freed, "files": count, "ok": True}
...
def clean_obs_cache():
    ...
    if os.path.isdir(obs_dir):
        for sub in ("crashes", "logs", "updates", "profiler_data"):
            p = os.path.join(obs_dir, sub)
            if os.path.isdir(p):
                targets.append(p)
                
    if not targets:
        warning("No OBS Studio cache found")
        return {"freed": 0, "files": 0, "ok": True}

    detail(f"Found {len(targets)} OBS Studio cache directories")
    freed, count = _parallel_delete_trees(targets)
    success(f"OBS Studio cache cleaned — {format_size(freed)} from {count:,} items")
    return {"freed": freed, "files": count, "ok": True}
...
def clean_gog_cache():
    ...
    gog_local = os.path.join(local, "GOG.com", "Galaxy")
    if os.path.isdir(gog_local):
        for sub in ("webcache", "logs"):
            p = os.path.join(gog_local, sub)
            if os.path.isdir(p):
                targets.append(p)
                
    gog_pd = os.path.join(program_data, "GOG.com", "Galaxy")
    if os.path.isdir(gog_pd):
        for sub in ("logs", "webcache"):
            p = os.path.join(gog_pd, sub)
            if os.path.isdir(p):
                targets.append(p)
                
    if not targets:
        warning("No GOG Galaxy cache found")
        return {"freed": 0, "files": 0, "ok": True}

    detail(f"Found {len(targets)} GOG Galaxy cache directories")
    freed, count = _parallel_delete_trees(targets)
    success(f"GOG Galaxy cache cleaned — {format_size(freed)} from {count:,} items")
    return {"freed": freed, "files": count, "ok": True}
...
TASKS = {
    ...
    "epic": ("Epic Games Cache", clean_epic_cache),
    "riot": ("Riot Games Cache", clean_riot_cache),
    "ea": ("EA/Origin Cache", clean_ea_cache),
    "battlenet": ("Battle.net Cache", clean_battlenet_cache),
    "ubisoft": ("Ubisoft Cache", clean_ubisoft_cache),
    "whatsapp": ("WhatsApp Cache", clean_whatsapp_cache),
    "obs": ("OBS Studio Cache", clean_obs_cache),
    "gog": ("GOG Galaxy Cache", clean_gog_cache),
}
...
DEEP_TASKS = [
    ...
    "telegram", "wer", "epic", "riot", "ea", "battlenet", "ubisoft",
    "whatsapp", "obs", "gog",
]
